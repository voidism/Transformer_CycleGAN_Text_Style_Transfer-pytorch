...
class EncoderDecoder(nn.Module):
    ...
    other models.
    """
    def __init__(self, encoder, decoder, src_embed, tgt_embed, generator, bridge):
    ...
    def conti_forward(self, src, tgt, src_mask, tgt_mask):
        "Take in and process masked src and target sequences."
        return self.conti_decode(self.conti_encode(src, src_mask), src_mask,
                            tgt, tgt_mask)
    
    def conti_encode(self, src, src_mask):
        return self.encoder(src, src_mask)
    
    def conti_decode(self, memory, src_mask, tgt, tgt_mask):
        return self.decoder(tgt, memory, src_mask, tgt_mask)

    def self_forward(self, src, src_mask, max_len, start_symbol=2):
        memory = self.encode(src, src_mask) # encode is discrete
        ys = torch.ones(src.shape[0], 1).fill_(start_symbol).type_as(src.data).to(device)
        collect_out = self.tgt_embed[0](ys).float()
        # collect_out.requires_grad = False
        # ret_out = self.tgt_embed[0](ys).float()
        for i in range(max_len-1):
            out = self.bridge(self.conti_decode(memory, src_mask, Variable(self.tgt_embed[1](collect_out)), 
                            Variable(subsequent_mask(collect_out.size(1))
                                        .type_as(src.data))))
            collect_out = torch.cat([collect_out, out[:, -1].unsqueeze(1)], dim=1)
            # ret_out = torch.cat([ret_out, out[:, -1].unsqueeze(1)], dim=1)
        #     # ret_out.append(out[:, -1].unsqueeze(1))
        #     print(i)
        # print("successfully getting out!")
        return collect_out[:, 1:]

    def semi_forward(self, src, src_mask, max_len, start_symbol=2):
        memory = model.encode(src, src_mask)
        ys = torch.ones(src.shape[0], 1).fill_(start_symbol).type_as(src.data).to(device)
        ret_out = self.tgt_embed[0](ys).float()
        for i in range(max_len-1):
            out = self.bridge(model.decode(memory, src_mask, 
                            Variable(ys), 
...
class Bridge(nn.Module):
    ...
    def forward(self, x):
        # return self.proj(x)
        return self.proj2(self.relu(self.proj1(x)))

class Generator(nn.Module):
    "Define standard linear + softmax generation step."
    def __init__(self, d_model, vocab, pretrained_mat=None):
        super(Generator, self).__init__()
        if not type(pretrained_mat) == type(None):
            self.proj = nn.Linear(d_model, vocab, bias=False)
            self.proj.weight.data = torch.tensor(pretrained_mat, dtype=torch.float32)
...
class my_generator(nn.Module):
    ...
    def forward(self, x):
        return self.proj(x) / torch.norm(self.proj.weight.data, dim=-1)*torch.norm(x)
...
def make_model(src_vocab, tgt_vocab, src_pre_trained_mat, tgt_pre_trained_mat, N=6, 
               d_model=1024, d_ff=2048, h=8, dropout=0.1, fix_gen=False, bridge=True):
    "Helper: Construct a model from hyperparameters."
    c = copy.deepcopy
    attn = MultiHeadedAttention(h, d_model)
    ff = PositionwiseFeedForward(d_model, d_ff, dropout)
    position = PositionalEncoding(d_model, 0)
    model = EncoderDecoder(
        ...
        my_generator(d_model, tgt_vocab, src_pre_trained_mat if fix_gen else None), Bridge(d_model)if bridge else None)
...
def data_gen_no_tf(iterator, sent2idx):
    "Generate random data for a src-tgt copy task."
    for i in iterator:
        data = torch.from_numpy(sent2idx(i)).long()
        # data = torch.from_numpy(np.random.randint(1, V, size=(batch, 10)))
        ...
        src = Variable(data, requires_grad=False)
        tgt = Variable(data, requires_grad=False)
        yield Batch_no_shift(src, tgt, 0)
...
class CELossCompute:
    ...
    def __call__(self, x, y, norm):
        x = self.generator(x)
        loss = self.criterion(x.transpose(1,2).to(device), 
                              y.to(device))
        loss.backward()
        if self.opt is not None:
            self.opt.step()
...
class L2LossCompute:
    ...
    def __call__(self, x, y, norm):
        loss = self.criterion(x.to(device), 
                              y.float().to(device))
        loss.backward()
        if self.opt is not None:
            self.opt.step()
...
class Utils():
    ...
    batch_size = 32):
        ...
        self.idx2word, self.word2idx, self.emb_mat = load_embedding(limit=100000)
        self.batch_size = batch_size
        self.X_step_num = math.floor(self.X_line_num / batch_size)
        self.Y_step_num = math.floor(self.Y_line_num / batch_size)
        self.device = device
    ...
    def most_similar_word(self, vec, lim=10000, topn=20):
        b = vec
        a = self.emb_mat[:lim,:]
        sim_array = np.matmul(a, b) / (np.linalg.norm(a, axis=1) * np.linalg.norm(b) + 1e-9)
        top_one = np.argmax(sim_array)
        return [(self.idx2word[top_one], 0)]

    def most_similar_words(self, vec, lim=10000, topn=20):
        b = vec
        a = self.emb_mat[:lim,:]
        ...
        ans = []
        for i in order:
            ans.append((self.idx2word[i], sim_array[i]))
        return ans

    def vecs2sent(self, vecs):
        vecs = vecs.detach().cpu().numpy()
        ret = []
        for seq in vecs:
            sent = []
            for vec in seq:
                possible_words = self.most_similar_word(vec,topn=1)
...
if __name__ == "__main__":
    ...
    L2_loss = nn.MSELoss()
    model = make_model(V, V, utils.emb_mat, utils.emb_mat, fix_gen=True, bridge=True)
    model.to(device)

    X_test_batch = None
    Y_test_batch = None
    for i, batch in enumerate(data_gen_no_tf(utils.data_generator("X"), utils.sents2idx)):
        X_test_batch = batch
        break

    for i, batch in enumerate(data_gen_no_tf(utils.data_generator("Y"), utils.sents2idx)):
        Y_test_batch = batch
        break
    ...
    if sys.argv[3] == 'noam':
        model_opt = NoamOpt(model.src_embed[0].d_model, 2, 4000,
                torch.optim.Adam(model.parameters(), lr=0, betas=(0.9, 0.98), eps=1e-9))
    elif sys.argv[3] == 'adam':
        model_opt = AdamOpt(torch.optim.Adam(model.parameters()))
    # model.train()
    # pretrain_without_tf(data_gen_no_tf(utils.data_generator("X"), utils.sents2idx), model, 
    #                 L2LossCompute(L2_loss, model_opt), utils.X_step_num)
    ...
    if sys.argv[1] == "train":
        for epoch in range(int(sys.argv[2])):
            ...
            pretrain_run_epoch(data_gen(utils.data_generator("Y"), utils.sents2idx), model, 
                    SimpleLossCompute(model.generator, criterion, model_opt), utils.Y_step_num, tf_rate=1.0-0.5*epoch)
            print("== Train Y ==")
            pretrain_run_epoch(data_gen(utils.data_generator("X"), utils.sents2idx), model, 
                    SimpleLossCompute(model.generator, criterion, model_opt), utils.X_step_num, tf_rate=1.0-0.5*epoch)
            model.eval()
            ...
            #                 SimpleLossCompute(model.generator, criterion, None), utils.X_step_num))
        print("=== training without teacher forcing ===")
        
        torch.save(model.state_dict(), 'model_v1.ckpt')
        
        for epoch in range(int(sys.argv[2])):
            model.train()
            print("EPOCH %d:" % (epoch + 1))
            print("== Train X ==")
            pretrain_without_tf(data_gen_no_tf(utils.data_generator("Y"), utils.sents2idx), model, 
                    L2LossCompute(L2_loss, model_opt), utils.Y_step_num, semi=True)
            print("== Train Y ==")
            pretrain_without_tf(data_gen_no_tf(utils.data_generator("X"), utils.sents2idx), model, 
                    L2LossCompute(L2_loss, model_opt), utils.X_step_num, semi=True)
            model.eval()

            x = utils.idx2sent(torch.max(model.generator(
                model.semi_forward(X_test_batch.src,
                X_test_batch.src_mask,
                max_len=20, start_symbol=2)), dim=-1)[1])
            y = utils.idx2sent(torch.max(model.generator(
                model.semi_forward(Y_test_batch.src,
                Y_test_batch.src_mask,
                max_len=20, start_symbol=2)), dim=-1)[1])
        ...
        print("=== training with free style ===")
        
        torch.save(model.state_dict(), 'model_v2.ckpt')

    if sys.argv[1] == "v3_only" or sys.argv[1] == "train":
        for epoch in range(int(sys.argv[2])):
            model.train()
            print("EPOCH %d:" % (epoch + 1))
            print("== Train X ==")
            ...
            # print("== Train X ==")
            # pretrain_without_tf(data_gen_no_tf(utils.data_generator("Y"), utils.sents2idx), model, 
            #         CELossCompute(model.generator, criterion, model_opt), utils.Y_step_num)
            # print("== Train Y ==")
            # pretrain_without_tf(data_gen_no_tf(utils.data_generator("X"), utils.sents2idx), model, 
            #         CELossCompute(model.generator, criterion, model_opt), utils.X_step_num)
            model.eval()
            model.to("cpu")
            device = "cpu"
            x = utils.vecs2sent(model.self_forward(X_test_batch.src,
                X_test_batch.src_mask,
                max_len=20, start_symbol=2))
            y = utils.vecs2sent(model.self_forward(Y_test_batch.src,
                Y_test_batch.src_mask,
                max_len=20, start_symbol=2))
            device = "cuda:0"
            model.to(device)
            ...
            for i, j in zip(Y_test_batch.src, y):
                ...
                print("GEN:", " ".join(j[:j.index('<eos>')+1] if '<eos>' in j else j))
                print("===")
            torch.save(model.state_dict(), 'model_v3.ckpt')
        torch.save(model.state_dict(), 'model_v3.ckpt')