...
class Generator(nn.Module):
    ...
    def forward(self, x, scale=1):
        return F.log_softmax(self.proj(x*scale), dim=-1)

    def backward(self, x):
        prob = F.log_softmax(self.proj(x), dim=-1)
        vec = self.proj.weight.data.transpose(0, 1) * prob
        return vec
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        x, self.attn = attention(query, key, value, mask=mask.to(device) if type(mask)!= type(None) else None, 
...
def make_model(src_vocab, tgt_vocab, src_pre_trained_mat, tgt_pre_trained_mat, N=6, 
               d_model=1024, d_ff=2048, h=8, dropout=0.1, fix_gen=False):
    ...
    model = EncoderDecoder(
        ...
        Generator(d_model, tgt_vocab, src_pre_trained_mat if fix_gen else None))
...
class Utils():
    ...
    batch_size = 32):
        ...
        self.idx2word, self.word2idx, self.emb_mat = load_embedding(limit=10000)
...
if __name__ == "__main__":
    ...
    model = make_model(V, V, utils.emb_mat, utils.emb_mat, fix_gen=False)
    model.to(device)
    ...
    if sys.argv[1] == "train":
        for epoch in range(int(sys.argv[2])):
            ...
            pretrain_run_epoch(data_gen(utils.data_generator("Y"), utils.sents2idx), model, 
                    SimpleLossCompute(model.generator, criterion, model_opt), utils.Y_step_num, tf_rate=(1.0-0.5*(epoch-10)) if epoch>10 else 0.0)
            print("== Train X ==")
            pretrain_run_epoch(data_gen(utils.data_generator("X"), utils.sents2idx), model, 
                    SimpleLossCompute(model.generator, criterion, model_opt), utils.X_step_num, tf_rate=(1.0-0.5*(epoch-10)) if epoch>10 else 0.0)
            model.eval()
            ...
            for i, j in zip(Y_test_batch.src, y):
                ...
                print("GEN:", " ".join(j[:j.index('<eos>')+1] if '<eos>' in j else j))
                print("===")
            torch.save(model.state_dict(), 'model_new.ckpt')
